...
SUPABASE_URL = os.getenv("SUPABASE_URL", "")  # kept for storage URLs if used
...
class _Query:
    ...
    def execute(self):
        ...
        if self._op == "select":
            ...
            if self._single:
                return _Resp(res[0] if res else None)
            return _Resp(res)
...
class _AuthAdmin:
    def create_user(self, payload):
        """Create a user via the custom auth API. Sends only email + password."""
        signup_payload = {
            "email": payload.get("email"),
            "password": payload.get("password"),
        }
        res = requests.post(f"{API_URL}/auth/v1/signup", json=signup_payload)
        data = res.json()

        user_id = data.get("user", {}).get("id", "local-user")
        ...
        # Return a container whose .user attribute holds the user object
        class _R:
            pass
        r = _R()
        r.user = u
...
# Replaces any old sb / sb_admin imports — only sb_admin is needed
...
def get_current_user():
    """
    Returns a simple user object whose .id is the stored access_token,
    because the custom API returns access_token == user_id.
...
def get_profile():
    """
    Fetches the profile row from the 'profiles' Supabase table using user_id.
    Returns a single dict (or None), never a list.
    """
    ...
    try:
        ...
        # maybe_single() now returns a single dict or None via _Query.execute()
        return res.data
    except Exception:
        return None
...
def register():
    if request.method == "POST":
        ...
        # Handle non-2xx or missing user
        if res.status_code not in (200, 201) or "user" not in data:
            error_msg = (
                data.get("message")
                or data.get("error")
                or data.get("msg")
            ...
            if "already" in str(error_msg).lower():
                ...
                return redirect(url_for("login"))
            flash("Sign up error: " + str(error_msg), "danger")
            return redirect(url_for("register"))
...
def login():
    if request.method == "POST":
        ...
        if res.status_code not in (200, 201) or "access_token" not in data:
            error_msg = (
                data.get("message")
                or data.get("error")
                or data.get("msg")
            ...
            flash("❌ Login failed: " + str(error_msg), "danger")
            return redirect(url_for("login"))

        # access_token == user_id in this API
...
def admin_delete_employee(user_id):
    ...
    # 2) Delete auth user (no-op in custom backend)
    try:
...
def send_appeal():
    ...
    if file and file.filename:
        ...
        if SUPABASE_URL:
            file_url = f"{SUPABASE_URL}/storage/v1/object/public/appeals/{file_name}"
    ...
    try:
        ...
        flash("✅ Appeal submitted.", "success")
    except Exception as e:
        flash("Appeal submission failed: " + str(e), "danger")
